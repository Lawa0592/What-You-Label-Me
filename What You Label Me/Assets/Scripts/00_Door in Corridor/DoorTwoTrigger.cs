...
/*
Introduction:
This script is responsible for listening to what happens when the player approaches the door two
1. Switch scene to scene '03_school_reality'
2.
3.
...
public class DoorTwoTrigger : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider collider)
    {
        if (collider.tag == "Player")  //checking if the collider is player's one
        {
            ...
            {
                if (!GameManager.isDoor2Locked) //only can open if the door isn't locked
                {
                    //sending message to SceneManager to change the scene
                    GameObject.Find("_Scene Manager").SendMessage("SceneReceiver", "03_school_reality");
                }
